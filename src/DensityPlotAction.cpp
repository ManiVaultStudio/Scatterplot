    ...
    _sigmaAction(this, "Sigma", 1.0, 50.0, DEFAULT_SIGMA, DEFAULT_SIGMA)
{
    ...
    const auto computeDensity = [this]() -> void {
        ...
        if (maxDensity > 0)
            _scatterplotPlugin->getSettingsAction().getColoringAction().getColorByDataAction().getColorMapAction().getSettingsAction().getHorizontalAxisAction().getRangeAction().setRange(0.0f, maxDensity);
    };
